...
<body>
    <form action="data.php" method="post">
        <fieldset>
            <legend>Edit Student Personal information:</legend>
            <lable>ชื่อ</lable><br>
            <input type="text" name="edit_name" required value="<?php echo $result['student_name'] ?>"><br>
            <lable>นามสกุล</lable><br>
            <input type="text" name="edit_lastname" required value="<?php echo $result['student_lastname'] ?>"><br>
            ...
            <?php
            if ($status == 0) {
                if ($result['advisor_id'] != null) {
                    echo ('<lable>ที่ปรึกษา</lable><br>
                    <input type="text" name="edit_teacherid" required value="' . $result['advisor_id'] . '">');
                }
            }
            ?><br>
            <p>
            <input type="hidden" name="edit_teacherid" value="<?php echo $result['advisor_id'] ?>">
    ...
    <div class="button-container">
        <?php if ($status == 1) echo ("<a href='TeacherMenu.php?pid=" . $id . "'><button>Teacher Menu</button></a>"); ?>
        <?php if ($status == 2) echo ("<a href='StudentMenu.php?pid=" . $id . "'><button>Student Menu</button></a>"); ?>
